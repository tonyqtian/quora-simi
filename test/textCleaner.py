...
#     text = text.replace("\\", " ")
    ...
    text = text.replace("what's", "what is ", text)
    text = text.replace("'s", " ", text)
    text = text.replace("'ve", " have ", text)
    text = text.replace("can't", "cannot ", text)
    text = text.replace("n't", " not ", text)
    text = text.replace("i'm", "i am ", text)
    text = text.replace("'re", " are ", text)
    text = text.replace("'d", " would ", text)
    text = text.replace("'ll", " will ", text)
    ...
    text = text.replace(":", " : ", text)
    text = text.replace(" e g ", " eg ", text)
    text = text.replace(" b g ", " bg ", text)
    text = text.replace(" u s ", " american ", text)
    # text = re.sub(r"\0s", "0", text)
    text = text.replace(" 9 11 ", "911", text)
    text = text.replace("e - mail", "email", text)
    # text = text.replace("j k", "jk", text)
